...
public class MessageUtil {
    ...
    public static String format(String string, InetAddress inetAddress) {
        ...
        result = result.replace("%remainPlayer%", WaiterManager.getInstance().get(inetAddress).toString());
        result = result.replace("%ip%",inetAddress.getHostName());
        result = result.replace("%priority%", WaiterManager.getInstance().get(inetAddress).toString());
        return result;
    }
    ...
    public static String formatWithoutPrefix(String string, InetAddress inetAddress, boolean useRemainTime) {
        ...
        if (useRemainTime) {
            result = result.replace("%remainTime%", String.valueOf(WaiterManager.getInstance().getRemainTime().getSeconds()));
        }
        return result;
    }
    ...
    public static String formatExtra(String string, InetAddress inetAddress, UUID uuid) {
        ...
        str = str.replace("%nickname%", WaiterManager.getInstance().getNickname(uuid));
        return str;
    }
